...
${stickerRotations.L[6]} ${stickerRotations.L[7]} ${stickerRotations.L[8]}   ${stickerRotations.F[6]} ${stickerRotations.F[7]} ${stickerRotations.F[8]}   ${stickerRotations.R[6]} ${stickerRotations.R[7]} ${stickerRotations.R[8]}   ${stickerRotations.B[6]} ${stickerRotations.B[7]} ${stickerRotations.B[8]}
      ...
      async function startCamera() {
        ...
        try {
          ...
          const processFrame = async () => {
            if (cameraEnabled && video.readyState >= 2 && faceMesh && mediaPipeReady) {
              try {
                await faceMesh.send({ image: video });
              } catch (e) {
                // Silently ignore MediaPipe processing errors
              }
            }
            if (cameraEnabled) {
      ...
      function onCameraResults(results) {
        if (results.multiFaceLandmarks && results.multiFaceLandmarks.length > 0) {
          ...
          if (isCalibrated) {
            ...
            // Apply to cube rotation (inverted X for opposite direction)
            cubeRotation.y = 0 - (deltaX * sensitivityX);
            cubeRotation.x = -15 + (deltaY * sensitivityY);
      ...
      function addMoveToHistory(move) {
        ...
        // Create new branch from selected node if different from current position
        if (selectedNodeIndex >= 0 && selectedNodeIndex !== currentHistoryIndex) {
          const newBranchName = `branch-${Date.now()}`;
          const newBranchHistory = moveHistory.slice(0, selectedNodeIndex + 1);
          
          const entry = { timestamp, move, state, branch: newBranchName, id: Date.now() };
          newBranchHistory.push(entry);
          
          branches[newBranchName] = newBranchHistory;
          currentBranch = newBranchName;
          moveHistory = [...newBranchHistory];
          currentHistoryIndex = moveHistory.length - 1;
          selectedNodeIndex = -1;
        } else {
          // Continue on current branch
          const entry = { timestamp, move, state, branch: currentBranch, id: Date.now() };
          moveHistory.push(entry);
          branches[currentBranch] = [...moveHistory];
          currentHistoryIndex = moveHistory.length - 1;
        }
      ...
      function updateHistoryDisplay() {
        ...
        try {
          gitgraph = GitgraphJS.createGitgraph(container, {
            orientation: 'horizontal'
          });
          
          const mainBranch = gitgraph.branch('main');
          const branchRefs = { main: mainBranch };
          
          // Build commit tree chronologically
          const allCommits = [];
          Object.keys(branches).forEach(branchName => {
            branches[branchName].forEach((entry, index) => {
              allCommits.push({ ...entry, branchName, index });
            });
          });
          
          // Sort by timestamp
          allCommits.sort((a, b) => a.id - b.id);
          
          // Add commits in chronological order
          allCommits.forEach(commit => {
            const { branchName, index, move } = commit;
            
            // Create branch if it doesn't exist
            if (!branchRefs[branchName]) {
              branchRefs[branchName] = gitgraph.branch(branchName.replace('branch-', 'b'));
            }
            
            // Determine commit color based on selection and current position
            const isSelected = selectedNodeIndex === index && branchName === currentBranch;
            const isCurrent = currentHistoryIndex === index && branchName === currentBranch;
            
            let dotColor = '#000';
            if (isSelected) dotColor = '#ff6b35';
            else if (isCurrent) dotColor = '#4caf50';
            
            // Add commit to appropriate branch
            branchRefs[branchName].commit({
              subject: move,
              dotColor: dotColor,
              onClick: () => selectNode(branchName, index)
            });
          });
          
        } catch (error) {
          console.error('GitGraph error:', error);
          container.innerHTML = '<div style="padding: 10px; font-size: 12px;">History: ' + 
            moveHistory.map(entry => entry.move).join(' → ') + '</div>';
        }
        
        // Update grid - show only current branch history
        ...
        moveHistory.forEach((entry, index) => {
          ...
          const isCurrent = currentHistoryIndex === index;
          
          let className = 'history-row';
          if (isSelected) className += ' selected';
          else if (isCurrent) className += ' current';
          
          row.className = className;
          row.onclick = () => selectNode(currentBranch, index);
          
          row.innerHTML = `
            <div class="history-cell">${entry.timestamp}</div>
            <div class="history-cell">${entry.move}</div>
            <div class="history-cell">${index + 1}</div>
          `;
      ...
      function selectNode(branchName, index) {
        // Switch to branch and set selected node
        currentBranch = branchName;
        moveHistory = [...branches[branchName]];
        selectedNodeIndex = index;
        currentHistoryIndex = index;
        
        const state = moveHistory[index].state;
      ...
      function scrambleCube() {
        ...
        moves.forEach((move) => applyMove(move, false));
